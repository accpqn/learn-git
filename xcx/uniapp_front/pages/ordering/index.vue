			...
			<view class="sidebar-wrapper">
				<scroll-view scroll-y class="category-scroll">
					<view
						...
						:style="category.id === menuStore.currentCategoryId ? categoryActiveStyle : {}"
						@click="handleCategoryChange(category.id)"
					>
						<text
							class="category-name"
							:style="category.id === menuStore.currentCategoryId ? categoryNameActiveStyle : {}"
						>{{ category.name }}</text>
						<view
						...
						>
							...
							<view class="product-right">
								<view class="product-info" @click="showProductDetail(product)">
									...
									<text class="product-price" :style="productPriceStyle">¥{{ product.price }}</text>
								</view>
								<view class="product-actions">
									...
									<button v-else class="add-btn" :style="addButtonStyle" @click.stop="addProductToCart(product)">
		...
		<u-transition :show="cartStore.totalItems > 0" mode="slide-up">
			<view class="cart-bar">
				<view class="cart-left" @click="showCartDetail">
					<view class="cart-icon-wrapper" :style="cartIconStyle">
						<text class="cart-icon">🛒</text>
						<view class="cart-badge" :style="cartBadgeStyle">{{ cartStore.totalItems }}</view>
					</view>
					<view class="cart-info">
						<text class="cart-text">{{ cartStore.totalItems }}件商品</text>
						<text class="total-price" :style="totalPriceStyle">¥{{ cartStore.totalPrice }}</text>
					</view>
				</view>
			...
			<view class="product-modal" v-if="selectedProduct" @click.stop>
				...
				<view class="modal-content">
					...
					<view class="product-detail">
						...
						<text class="detail-price" :style="productPriceStyle">¥{{ selectedProduct.price }}</text>
					</view>
				</view>
...
export default {
	setup() {
		...
		const categoryNameActiveStyle = computed(() => ({
			color: currentTheme.value.primaryColor
		}));

		const productCountStyle = computed(() => ({
		...
		const checkoutButtonStyle = computed(() => ({
			background: `linear-gradient(135deg, ${currentTheme.value.primaryColor}, ${currentTheme.value.secondaryColor})`
		}));
