  ...
  setup() {
    ...
    const refreshOrders = async () => {
      if (userStore.isLoggedIn) {
        common_vendor.index.__f__("log", "at pages/order/list.vue:168", "刷新订单数据...");
        try {
          await Promise.all([
            orderStore.fetchSentOrders(),
            orderStore.fetchReceivedOrders()
          ]);
          common_vendor.index.__f__("log", "at pages/order/list.vue:174", "订单数据刷新完成");
        } catch (error) {
          common_vendor.index.__f__("error", "at pages/order/list.vue:176", "刷新订单数据失败:", error);
        }
      }
    };
    ...
    common_vendor.onShow(() => {
      common_vendor.index.__f__("log", "at pages/order/list.vue:192", "订单页面显示");
      if (userStore.isLoggedIn) {
        refreshOrders();
      }
    });
    ...
    const tabActiveStyle = common_vendor.computed(() => ({
  ...
  return common_vendor.e({
    ...
    c: common_vendor.f($setup.tabList, (tab, index, i0) => {
      return {
        ...
        d: common_vendor.s($setup.currentTab === index ? $setup.tabActiveStyle : {}),
        e: common_vendor.o(($event) => $setup.handleTabClick({
          index
        }), index)
      };
    }),
    ...
    g: common_vendor.s($setup.totalPriceStyle),
    h: $setup.displaySentOrders.length === 0
  }, $setup.displaySentOrders.length === 0 ? {} : {}), {
    i: $setup.isLoggedIn && $setup.orderStore.status === "loading"
  }, $setup.isLoggedIn && $setup.orderStore.status === "loading" ? {} : common_vendor.e({
    j: common_vendor.f($setup.displayReceivedOrders, (order, k0, i0) => {
      return common_vendor.e({
        a: common_vendor.t(order.id),
        b: common_vendor.t($setup.getStatusText(order.status)),
        c: common_vendor.n($setup.getStatusClass(order.status)),
    ...
    k: common_vendor.s($setup.totalPriceStyle),
    l: $setup.displayReceivedOrders.length === 0
  }, $setup.displayReceivedOrders.length === 0 ? {} : {}), {
    m: $setup.currentTab,
    n: common_vendor.o((...args) => $setup.handleSwiperChange && $setup.handleSwiperChange(...args))
  });
}
