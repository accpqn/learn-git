...
const useUserStore = common_vendor.defineStore("user", {
  ...
  actions: {
    ...
    async fetchCurrentUser() {
      if (!this.token) {
        common_vendor.index.__f__("log", "at store/user.js:185", "No token available, skipping fetchCurrentUser");
        return false;
      }
      if (this._fetchingUser) {
        common_vendor.index.__f__("log", "at store/user.js:191", "Already fetching user info, skipping duplicate request");
        return false;
      }
      this._fetchingUser = true;
      try {
        const coupleStore = store_couple.useCoupleStore();
        common_vendor.index.__f__("log", "at store/user.js:199", "Fetching current user info...");
        const response = await api_user.getCurrentUser();
        common_vendor.index.__f__("log", "at store/user.js:201", "Received user info:", response);
        const { binding, ...userData } = response;
        this.setUserInfo(userData);
        coupleStore.setBindingInfo(binding);
        common_vendor.index.__f__("log", "at store/user.js:208", "User info updated successfully");
        return true;
      } catch (error) {
        common_vendor.index.__f__("error", "at store/user.js:211", "Failed to fetch current user:", error);
        if (error.response && error.response.statusCode === 401) {
          common_vendor.index.__f__("log", "at store/user.js:214", "Token expired, clearing user state");
          this.clearUserState();
        }
        return false;
      } finally {
    ...
    syncThemeAfterLogin() {
      ...
      try {
        ...
        common_vendor.index.__f__("log", "at store/user.js:252", "已发送登录成功事件，主题将自动同步");
      } catch (error) {
        common_vendor.index.__f__("error", "at store/user.js:254", "同步主题配置失败:", error);
      }
    }
  }
});
