  ...
  actions: {
    ...
    async fetchThemeFromServer() {
      ...
      try {
        ...
        if (error.message && error.message.includes("401")) {
          ...
          common_vendor.index.__f__("log", "at store/theme.js:128", "新用户没有绑定关系，使用默认主题");
          this.applyTheme();
        } else {
          common_vendor.index.__f__("log", "at store/theme.js:131", "其他错误，使用本地主题配置");
          this.applyTheme();
        }
      } finally {
        this.loading = false;
    ...
    async saveThemeToServer() {
      ...
      try {
        ...
        this.saveToStorage();
        common_vendor.index.__f__("log", "at store/theme.js:158", "主题配置已保存到服务器");
      } catch (error) {
        common_vendor.index.__f__("error", "at store/theme.js:160", "保存主题配置到服务器失败:", error);
        this.saveToStorage();
        throw error;
      } finally {
        this.loading = false;
    ...
    applyTheme() {
      try {
        ...
        this._applyThemeTimer = setTimeout(() => {
          ...
          common_vendor.index.__f__("log", "at store/theme.js:212", "主题已应用:", this.currentTheme);
        }, 50);
      } catch (error) {
        common_vendor.index.__f__("error", "at store/theme.js:215", "应用主题失败:", error);
      }
    },
    // 保存到本地存储
    saveToStorage() {
      try {
        ...
        common_vendor.index.__f__("error", "at store/theme.js:224", "保存主题到本地存储失败:", error);
      }
    },
    // 从本地存储加载
    loadFromStorage() {
      try {
        ...
        common_vendor.index.__f__("error", "at store/theme.js:234", "从本地存储加载主题失败:", error);
        return null;
      }
    },
    // 检查是否已登录
