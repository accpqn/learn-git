...
const _sfc_main = {
  setup() {
    ...
    common_vendor.onMounted(() => {
      common_vendor.index.__f__("log", "at pages/user/index.vue:168", "User page onMounted - isLoggedIn:", userStore.isLoggedIn);
    });
    common_vendor.onShow(() => {
      common_vendor.index.__f__("log", "at pages/user/index.vue:172", "User page onShow - isLoggedIn:", userStore.isLoggedIn);
      common_vendor.index.__f__("log", "at pages/user/index.vue:173", "User page onShow - userInfo:", userStore.userInfo);
      common_vendor.index.__f__("log", "at pages/user/index.vue:174", "User page onShow - token:", userStore.token);
      if (userStore.isLoggedIn && (!userStore.userInfo || !userStore.userInfo.id)) {
        common_vendor.index.__f__("log", "at pages/user/index.vue:178", "Refreshing user info on page show");
        userStore.fetchCurrentUser();
      }
      orderStore.initOrders();
    });
    common_vendor.watch(() => userStore.isLoggedIn, (newVal, oldVal) => {
      common_vendor.index.__f__("log", "at pages/user/index.vue:187", "Login status changed:", oldVal, "->", newVal);
    }, { immediate: true });
    const goToLogin = () => {
    ...
    const changeAvatar = () => {
      common_vendor.index.chooseImage({
        ...
        success: async (res) => {
          ...
          common_vendor.index.__f__("log", "at pages/user/index.vue:219", "选择的头像文件:", tempFilePath);
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    l: common_vendor.t($setup.orderStats.received),
    m: common_vendor.t($setup.orderStats.total),
    n: common_vendor.o((...args) => $setup.editProfile && $setup.editProfile(...args)),
    o: common_vendor.t($setup.bindingInfo && $setup.bindingInfo.status === "active" ? "情侣管理" : "绑定情侣"),
    p: common_vendor.o((...args) => $setup.handleCoupleAction && $setup.handleCoupleAction(...args)),
    q: common_vendor.o((...args) => $setup.navigateToConfig && $setup.navigateToConfig(...args)),
    r: common_vendor.o((...args) => $setup.showAbout && $setup.showAbout(...args)),
    s: common_vendor.o((...args) => $setup.clearData && $setup.clearData(...args)),
    t: common_vendor.s($setup.getButtonStyle("outline")),
    v: common_vendor.o((...args) => $setup.handleLogout && $setup.handleLogout(...args))
  }));
}
