...
function sendBindingRequest(email) {
  return api_request.request({
...
function uploadAvatar(filePath) {
  return new Promise((resolve, reject) => {
    ...
    common_vendor.index.uploadFile({
      url: `${define_import_meta_env_default.VITE_API_BASE_URL || "http://127.0.0.1:8000"}/api/users/upload-avatar/`,
      filePath,
      name: "avatar",
      header: {
      ...
      success: (res) => {
        common_vendor.index.__f__("log", "at api/user.js:103", "头像上传响应:", res);
      ...
      fail: (error) => {
        common_vendor.index.__f__("error", "at api/user.js:116", "头像上传失败:", error);
        reject(error);
      }
    });
  });
